...
use crate::charts::{ChartConfig, FilterConfig, FilterSpec};
...
fn build_py_chart_config(m: &Bound<'_, PyDict>, config: &ChartConfig) -> PyResult<()> {
    match config {
...
fn build_py_module<'py>(py: Python<'py>, module: &PageModule) -> PyResult<Bound<'py, PyDict>> {
    ...
    match module {
        PageModule::Chart(spec) => {
            ...
            build_py_chart_config(&m, &spec.config)?;
        }
